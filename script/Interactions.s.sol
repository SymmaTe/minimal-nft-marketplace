...
contract ListNftMarketplace is Script {
    ...
    function run() external {
        address nftMarketplaceAddress = DevOpsTools.get_most_recent_deployment("NftMarketplace", block.chainid);
        listNft(nftMarketplaceAddress, NFT_CONTRACT, TOKEN_ID, LISTING_PRICE);
    }

    function listNft(address nftMarketplaceAddress, address nftAddress, uint256 tokenId, uint256 price) public {
        vm.startBroadcast();
        NftMarketplace(nftMarketplaceAddress).listItem(nftAddress, tokenId, price);
        vm.stopBroadcast();
    }
}
...
contract CancelNftListing is Script {
    ...
    function run() external {
        address nftMarketplaceAddress = DevOpsTools.get_most_recent_deployment("NftMarketplace", block.chainid);
        cancelListing(nftMarketplaceAddress, NFT_CONTRACT, TOKEN_ID);
    }

    function cancelListing(address nftMarketplaceAddress, address nftAddress, uint256 tokenId) public {
        vm.startBroadcast();
        NftMarketplace(nftMarketplaceAddress).cancelListing(nftAddress, tokenId);
        vm.stopBroadcast();
    }
}
